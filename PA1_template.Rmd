...
The following code was used to load the data into R.  The data was stored in a data table called **Actdata** and loaded directly from the zipped file.
...
The **mean** and **median** number of steps was calculated using the following statements which catered for the 'NA' values in the **steps** column.
...
The interval **835** contains the maximum average number of steps (about 206).


## 4. Imputing missing values
In this dataset missing information was denoted by NA. Sometimes missing information can introduce bias into calculations or summaries of data.  As a result the NA's were replaced for further analysis. 
...
The new dataset with no missing data was created by selecting all columns from the original data, except the **steps** columns which contains NAs.  The new column **imputSteps** replaced **steps**.
...
The total steps per day was calculated using the data with no missing values, newdata, and a histogram was created.
...
The new mean and median of total steps taken per day are provided below.
```{r newmeanmedian}
        mean(newSumStepsDay$steps, na.rm = TRUE)
        median(newSumStepsDay$steps, na.rm = TRUE)
```

The mean value remained the same at **10766.19** for the original data and the complete data.  The median increased only slightly from 10765 to **10766.19**.  Note that with the new data where no NA's are present, both the mean and the median are identical. 

Therefore it seems as if imputing the missing data had no effect on the estimates of the total daily number of steps. 
...
These section checks whether daily activity differs on weekends compared to weekdays.
...
### 5.2 Make a panel plot containing a time series plot of interval and averagesteps taken
The new data with no missing data was grouped by the interval then day category.  The average steps was re-calculated for each group across all days.
